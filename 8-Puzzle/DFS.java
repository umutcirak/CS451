...
public class DFS {
    ...
    public static Stack<Node> fringe;
    ...
    public DFS(int[] goal_state) {
        this.fringe =  new Stack<Node>();
    ...
    public void search(Node root){
        ...
        while (!fringe.isEmpty() && !is_found){
            ...
            for(Node child: node.children){
                if(!contains(fringe, child) && !check_Visited(child)){ // ! visited && fringe has child
    ...
    public boolean contains(Stack<Node> stack , Node n){
        boolean contains = false;
        for (int i = 0; i < stack.size(); i++) {
            if( stack.get(i).is_Same_State(n.state)  ){
                contains = true;
                break;
            }
        }
